...
req () { # req <method> <url> <token> [json] -> sets STATUS and BODY
  local auth=()
...
req POST $API/auth/login "" '{"email":"dylan@example.com","password":"dylan-pass-123"}'
check "login dylan" 200 "$STATUS"
DYLAN=$(json "['access_token']")
req POST $API/auth/login "" '{"email":"eleanor@example.com","password":"eleanor-pass-123"}'
check "login eleanor" 200 "$STATUS"
ELEANOR=$(json "['access_token']")
...
FOUND=$(json " and len(json.loads('$BODY'))" 2>/dev/null || echo "$BODY" | /usr/bin/python3 -c "import sys, json; print(len(json.load(sys.stdin)))")
...
MITCH_ID=$(json "['id']")
req POST $API/auth/login "" '{"email":"mitch@example.com","password":"mitch-pass-123"}'
MITCH=$(json "['access_token']")
